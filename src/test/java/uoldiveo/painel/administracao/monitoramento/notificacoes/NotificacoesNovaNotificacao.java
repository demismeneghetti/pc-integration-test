...
public class NotificacoesNovaNotificacao {
    ...
    public void selecionaTipoNotificacaoEmail(int email){
 	...
 	List<WebElement> cxContatosSelecionados = driver.findElement(By.id("selectedUsers")).findElements(By.name("DEMIS MENEGHETTI"));
 	System.out.println(cxContatosSelecionados.get(email).getText());
 	// cxContatosSelecionados.get(email).click();
 	
    }
    
    public void selecionaTipoNotificacaoSMS(int sms){
	
 	List<WebElement> cxContatosSelecionados = driver.findElement(By.tagName("td")).findElements(By.className("usersEmailNotification"));
 	cxContatosSelecionados.get(sms).click();
	...
	// BOTAO SELECIONE
//	WebElement btSelect = driver.findElement(By.className("setAvailableSelectedItems"));
//	btSelect.click();
...
//	List<WebElement> cxContatoSelecione = driver.findElements(By.id("availableUsers"));
//	cxContatoSelecione.get(usuario - 1);
//
//	List<WebElement> cxContatoSelecionado = driver.findElements(By.id("availableUsers"));
//	cxContatoSelecionado.get(usuario - 1).findElement(By.className("usersEmailNotification")).click();
//	cxContatoSelecionado.get(usuario - 1).findElement(By.className("usersSmsNotification")).click();
//
//	WebElement btCriarNotificacao = driver.findElement(By.className("submitForm"));
